...
func Map(value string) *list.List {
	// get words
	fun := func(x rune) bool{
		return !unicode.IsLetter(x)
	}
	get_words := strings.FieldsFunc(value, fun)

	// add the words into map
	add := list.New()
	for _, wd := range get_words{
		add.PushBack(mapreduce.KeyValue{wd, "1"})
	}
	return add
}
...
func Reduce(key string, values *list.List) string {
	// keep a count of the words
	ct := 0
	for i :=  values.Front(); i != nil; i = i.Next(){
		vals, err := strconv.Atoi(i.Value.(string))
		if err != nil{
			log.Fatal(err)
		}
		ct += vals
